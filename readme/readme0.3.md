...
| **Datasheet YAML** | Cuotas, caudales, carga de trabajo y políticas de consumo a nivel de endpoint | Siempre (comercial o no) |
...
| `syntax_version` | Versión del esquema | `0.3` |
| `date` | Fecha de extracción | `YYYY-MM-DD` |
| `currency` | Divisa utilizada en todos los precios del documento | `USD` · `EUR` · `GBP` |
| `consumption_units` | Lista de unidades de consumo que maneja el servicio | Lista de strings: `emails` · `requests` · `sms` · `tokens` · etc. |
...
| `period` | Ventana temporal | `value: 1` + `unit: MONTH` |
| `window_type` | Tipo de reinicio | `rolling` · `fixed_utc_midnight` · `fixed_billing_cycle` |
| `overage_cost` | _(opcional)_ Coste por exceso | Objeto con `price` y `value` |
...
| `period` | Ventana temporal | `value: 1` + `unit: MIN` |
...
| Campo | Descripción |
|---|---|
| `price` | Precio del plan (numérico o `custom`), en la divisa declarada en `currency` |
| `period` | Periodo de facturación (`value` + `unit`) |
...
#### 3.4.2 `workload` — Carga de trabajo por petición

El campo `workload` describe de forma estructurada **cuántas unidades de consumo genera una sola petición** a ese endpoint, expresando el rango posible.

| Campo | Tipo | Descripción |
|---|---|---|
| `unit` | `string` | Unidad que se consume por petición (`emails` · `sms` · `tokens` · `requests`) |
| `min` | `integer` | Mínimo de unidades consumidas por petición |
| `max` | `integer` | Máximo de unidades consumidas por petición |
| `description` | `string` | Explicación de qué determina el valor real dentro del rango |
...
#### 3.4.3 Aliases — Modos de consumo dentro de un endpoint

Un mismo endpoint puede tener **diferentes comportamientos o límites** según la forma en que el usuario lo consume. Estos se modelan como **aliases** (subniveles) dentro del endpoint, al mismo nivel que `quota` y `workload`.
...
Los aliases conviven directamente bajo la clave del endpoint, al mismo nivel que `quota` y `workload`.

**Posición de `quota` cuando hay aliases — depende de si la comparten:**

- **Todos los aliases comparten la misma cuota** → `quota` se declara a nivel del endpoint, por encima de los aliases:

```yaml
v1/email:
  quota: quota_500_emails_month       # compartida por todos los aliases
  workload:
    unit: emails
    min: 1
    max: 70
...
- **Caso mixto** — algunos aliases comparten cuota y otro tiene la suya propia: la cuota compartida va a nivel del endpoint y el alias diferente la sobreescribe con la suya dentro de su bloque:

```yaml
v1/email:
  quota: quota_emails_standard        # compartida por healthy y priority
  workload:
    unit: emails
    min: 1
    max: 70
  ...
  under_review_reputation:
    rate: rate_10_min
    quota: quota_emails_review        # cuota propia, distinta de la compartida
```

**Sin aliases:** Si el endpoint no tiene modos diferenciados, los campos `rate` y `quota` van directamente bajo la clave del endpoint.
...
- **`period`:** Objeto con `value` (número) y `unit` (string en mayúsculas) → `value: 1` + `unit: MONTH` / `unit: DAY` / `unit: MIN`.
- **`window_type`:** `rolling` | `fixed_utc_midnight` | `fixed_billing_cycle`.
- **`unit`:** `requests` · `emails` · `sms` · `tokens` · `records` · `gb` · `hours` · `minutes`.
- **`currency`:** Código ISO 4217 en mayúsculas → `USD` · `EUR` · `GBP`.
...
syntax_version: 0.3
date: # YYYY-MM-DD
currency: # Código ISO 4217 — USD | EUR | GBP
consumption_units:
  - # unidad 1 — emails | requests | sms | tokens | ...
  - # unidad 2
...
plans:
  ...
  #       rate:
  #
  #     ruta/sin/aliases:  # endpoint simple (sin modos diferenciados)
  #       rate:
  #       quota:
  #       workload:
  #         unit:
  ...
  #         description:
  #
  #     ruta/con/aliases:  # endpoint con modos de consumo diferenciados
  #       quota:           # aquí si TODOS los aliases comparten la misma cuota
  #       workload:        # carga de trabajo por petición
  #         unit:
  #         min:
  #         max:
  #         description:
  #       alias_1:         # p.ej. healthy_reputation
  #         rate:
  #         quota:         # aquí si este alias tiene su propia cuota (distinta de los demás)
  #       alias_2:
  #         rate:
  #         quota:         # ídem — omitir si usa la cuota declarada a nivel de endpoint
```
...
| Campo / Concepto | v0.1 | v0.2 | v0.3 |
|---|---|---|---|
| `currency` | — | — | **Nuevo** — divisa global del documento |
| `consumption_units` | — | — | **Nuevo** — declaración de unidades del servicio |
| `cost_per_request` | string libre | objeto `{value, unit}` | **Eliminado** → sustituido por `workload` |
| `workload` | — | — | **Nuevo** — objeto `{unit, min, max, description}` |
| Wildcard `/*` | — | Sí | Sí |
| Aliases en endpoints | — | Sub-objeto bajo `aliases:` | **Aplanados** — al mismo nivel que `quota` y `workload` |
| `period` en planes | string (`1 month`) | string (`1 month`) | **Objeto** `{value, unit}` (coherente con `capacity`/`max_power`) |
...
- El campo `workload` modela el rango de consumo por petición. Cuando el consumo es siempre exactamente 1 unidad, `min` y `max` serán ambos `1`.
- El esquema es **preliminar (v0.3)**: hay APIs donde habrá que adaptar o extender los campos según lo que se descubra en la documentación.
